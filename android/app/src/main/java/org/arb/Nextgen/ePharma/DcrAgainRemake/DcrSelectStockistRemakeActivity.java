...
import org.arb.Nextgen.ePharma.adapter.DcrRemakeAdapter.DcrSelectStockistRemakeAdapter;
import org.arb.Nextgen.ePharma.adapter.DcrRemakeAdapter.DcrStockistWorkPlaceRemakeAdapter;
...
public class DcrSelectStockistRemakeActivity extends AppCompatActivity implements View.OnClickListener {
    Button  btn_load_stockist, btn_cancel, btn_add;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dcr_select_stockist_remake);
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.btn_load_stockist:
                if (DcrStockistWorkPlaceRemakeAdapter.dcrDctrChemistStockistWorkPlaceModelArrayList.size() < 0) {
...
//            JSONArray jsonArray1 = jsonObject.getJSONArray("work_place_stockist");
            ...
            recycler_view_worked_place.setAdapter(new DcrStockistWorkPlaceRemakeAdapter(this, dctrChemistStockistWorkPlaceModelArrayList));
    ...
    public void loadStockistName() {
        ...
        loadStockist_to_retain_previous_data();
        String id = "";
        String hqId = "";
        for (int i = 0; i < DcrStockistWorkPlaceRemakeAdapter.dcrDctrChemistStockistWorkPlaceModelArrayList.size(); i++) {
            if (DcrStockistWorkPlaceRemakeAdapter.dcrDctrChemistStockistWorkPlaceModelArrayList.get(i).getStatus().contains("1")) {
                id = id + DcrStockistWorkPlaceRemakeAdapter.dcrDctrChemistStockistWorkPlaceModelArrayList.get(i).getId() + ",";
            }
        }
        if(id!="") {
...
//        String url = Config.BaseUrlEpharma + "MSR/Customer-List/" + userSingletonModel.getUser_id() + "/" + hqId + "/stockist/7";
    ...
    public void getResponseData (String response){
        try {
            ...
            recycler_view_select_stockist.setAdapter(new DcrSelectStockistRemakeAdapter(this, dcrSelectDoctorStockistChemistModelArrayList));
    ...
    public void saveToArrayList(){
        ...
        try {
            ...
            if (!DcrSelectStockistRemakeAdapter.dcrSelectDoctorStockistChemistModelArrayList.isEmpty()) {
                for (int i = 0; i < DcrSelectStockistRemakeAdapter.dcrSelectDoctorStockistChemistModelArrayList.size(); i++) {
                    if (DcrSelectStockistRemakeAdapter.dcrSelectDoctorStockistChemistModelArrayList.get(i).getStatus().contentEquals("1")) {
                        JSONObject reqObj = new JSONObject();
                        reqObj.put("name", DcrSelectStockistRemakeAdapter.dcrSelectDoctorStockistChemistModelArrayList.get(i).getName());
                        reqObj.put("id", DcrSelectStockistRemakeAdapter.dcrSelectDoctorStockistChemistModelArrayList.get(i).getId());
                        reqObj.put("work_place_id", DcrSelectStockistRemakeAdapter.dcrSelectDoctorStockistChemistModelArrayList.get(i).getWork_place_id());
                        reqObj.put("ecl_no", DcrSelectStockistRemakeAdapter.dcrSelectDoctorStockistChemistModelArrayList.get(i).getEcl_no());
                        reqObj.put("status", DcrSelectStockistRemakeAdapter.dcrSelectDoctorStockistChemistModelArrayList.get(i).getStatus());
...
//                sqliteDb.createDatabase();
                ...
                Toast.makeText(getApplicationContext(),"Please select atleast one stockist",Toast.LENGTH_LONG).show();
            }
        }catch (JSONException e){
            e.printStackTrace();
        }
    ...
    public void loadStockist_to_retain_previous_data(){
        //----stockist
        String jsonStockist = "";
